...
export default {
  ...
  props: {
    ...
    keyPress: {
      type: Array,
      default() {
        return [38, 40, 37, 39, 13] // 上下左右 13是回车
      },
    },
    // 初始index
    index: {
  ...
  methods: {
    ...
    onKeyDown(e) {
      ...
      const [top, bottom, left, right, enter] = this.keyPress
      const type = {
        [top]: 'top', // 上
        [bottom]: 'bottom', // 下
        [left]: 'left', // 左
    ...
    move(type) {
      ...
      if (type === 'enter') this.leftOrEnterMove(type)
    },
    // 上一次元素改为无选中状态
    lastTimeLi() {
    ...
    leftOrEnterMove(type, index) {
      ...
      // 最终发送确认值
      this.list[nextIndex].select = true
      this.currentIndex = nextIndex
      const background = this.background
      if (background) {
        const dom = this.list[nextIndex].self.$el
        dom.style.background = this.background
      }

      this.$emit('enterDate', this.list[nextIndex], this.currentIndex)
      // this.sendEmit()
    },
    // 左右移动
